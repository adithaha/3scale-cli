...
var  threescale_waterfall= function(api, service_id, appplan_name,method_pattern){
  appplan.getApplicationPlanByName(service_id,appplan_name).then(function(plan){
    if(plan)
      return;
    return appplan.createApplicationPlan(service_id,appplan_name).then(function(plan){
        var application_plan_id = plan.id;
  ...
  .then(function(metricList){
    cli.print({message: "Hits metric with id "+ metricList[0].metric.id+" found on 3scale"});
    HIT_METRIC_ID = metricList[0].metric.id;
    return metricList[0].metric.id;
